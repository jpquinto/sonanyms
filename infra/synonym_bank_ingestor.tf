module "word_bank_ingestion_lambda" {
  ...
  name            = "synoynm-word-bank-ingestion-lambda"
  handler         = "handler.handler"
  source_dir      = "${path.root}/../lambda_functions/dist/word_bank_ingestion/synonyms"
  build_path      = "${path.root}/../lambda_functions/build/word_bank_ingestion/synonyms/word_bank_ingestion.zip"
  runtime         = "nodejs20.x"
  memory          = 256
  time_limit      = 60
  deployment_type = "zip"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = ["${module.s3_bucket.bucket_arn}/synonyms/*"]
      },
      {
        Effect = "Allow",
        Action = [
